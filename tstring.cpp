...
void copyFromUTF8(std::wstring &data, const char *s, size_t length) {
  ...
  try {
    const std::wstring::iterator dstEnd =
        utf8::utf8to16(s, s + length, data.begin());
...
template <typename T> unsigned short nextUTF16(const T **p);

template <> unsigned short nextUTF16<wchar_t>(const wchar_t **p) {
  return static_cast<unsigned short>(*(*p)++);
}

template <> unsigned short nextUTF16<char>(const char **p) {
  union {
    unsigned short w;
    char c[2];
  } u;
...
template <typename T>
void copyFromUTF16(std::wstring &data, const T *s, size_t length,
                   String::Type t) {
  bool swap;
  if (t == String::UTF16) {
    if (length < 1) {
      debug("String::copyFromUTF16() - Invalid UTF16 string. Too short to have "
            "a BOM.");
      return;
    }
...
String::String() : d(std::make_shared<StringPrivate>()) {}
...
String::String(const std::wstring &s) : String(s, wcharByteOrder()) {}
...
String::String(const std::wstring &s, Type t)
  ...
  if (t == UTF16 || t == UTF16BE || t == UTF16LE) {
    ...
    debug(
        "String::String() -- std::wstring should not contain Latin1 or UTF-8.");
  }
}

String::String(const wchar_t *s) : String(s, wcharByteOrder()) {}
...
String::String(const wchar_t *s, Type t)
  ...
  if (t == UTF16 || t == UTF16BE || t == UTF16LE) {
    ...
    debug("String::String() -- const wchar_t * should not contain Latin1 or "
          "UTF-8.");
  }
}

String::String(const char *s, Type t) : d(std::make_shared<StringPrivate>()) {
  if (t == Latin1)
...
String::String(wchar_t c, Type t) : d(std::make_shared<StringPrivate>()) {
  if (t == UTF16 || t == UTF16BE || t == UTF16LE)
...
String::String(char c, Type t) : d(std::make_shared<StringPrivate>()) {
  if (t == Latin1)
...
std::wstring String::toWString() const { return d->data; }
...
const wchar_t *String::toCWString() const { return d->data.c_str(); }
...
String::ConstIterator String::begin() const { return d->data.begin(); }

String::ConstIterator String::cbegin() const { return d->data.cbegin(); }
...
String::ConstIterator String::end() const { return d->data.end(); }

String::ConstIterator String::cend() const { return d->data.cend(); }
...
bool String::isEmpty() const { return d->data.empty(); }
...
String String::stripWhiteSpace() const {
  ...
  return substr(static_cast<unsigned int>(pos1),
                static_cast<unsigned int>(pos2 - pos1 + 1));
}
...
const wchar_t &String::operator[](int i) const { return d->data[i]; }
...
bool String::operator==(const wchar_t *s) const { return d->data == s; }

bool String::operator!=(const wchar_t *s) const { return !(*this == s); }
...
bool String::operator<(const String &s) const { return d->data < s.d->data; }
